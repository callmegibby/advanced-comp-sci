function SearchTreeTypescript(e, l) {
	return (
		null !== e &&
		(e.value === l ||
			(e.value > l
				? SearchTreeTypescript(e.left, l)
				: SearchTreeTypescript(e.right, l)))
	)
}
function SearchListTypescript(e, l) {
	for (var r = 0, t = e.length - 1; r <= t; ) {
		var a = r + Math.floor((t - r) / 2),
			c = e[a]
		if (c === l) return a
		c > l ? (t = a - 1) : (r = a + 1)
	}
...
var chalk = require('chalk'),
	list = [1, 5, 7, 9, 11, 15, 17],
	head = {
		value: 10,
		left: {
			value: 8,
			left: { value: 3, right: null, left: { value: 1, right: null, left: null } },
